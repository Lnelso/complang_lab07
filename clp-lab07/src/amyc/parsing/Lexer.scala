  ...
  private def lexFile(ctx: Context)(f: File): Stream[Token] = {
    ...
    def nextToken(stream: Stream[Input]): (Token, Stream[Input]) = {
      ...
      if (Character.isWhitespace(currentChar)) {
        ...
        // Multi-line comment
        def goToNextStar(streamIn: Stream[Input]): Stream[Input] = {
          val untilStar = streamIn.dropWhile(_._1 != '*' )
          if(untilStar.isEmpty || untilStar.tail.isEmpty || untilStar.head._1 == EndOfFile) //Never found the end of the comment
            ctx.reporter.fatal("Comment is never closed.", currentPos)
          else if(untilStar.tail.head._1 == '/') untilStar.drop(2)
          else goToNextStar(untilStar.tail)
        }
        nextToken(goToNextStar(rest.tail))
      } else {
        readToken(stream)
      }
    }
    ...
    def readToken(stream: Stream[Input]): (Token, Stream[Input]) = {
      ...
      currentChar match {
        ...
        case _ if Character.isLetter(currentChar) =>
          ...
          keywords(word) match {
            case Some(x) => (x.setPos(currentPos), afterWord)
        ...
        case _ if Character.isDigit(currentChar) =>
          ...
          val (number, afterInt) = stream.span { case (nb, _) =>
            Character.isDigit(nb)
          }
          val numberString = number.map(_._1).mkString
          try{
            (INTLIT(numberString.toInt).setPos(currentPos), afterInt)
          }
          catch{
            case e: NumberFormatException => {
              ctx.reporter.error("Overflow", currentPos)
        ...
        // String literal
        case '"' =>
          val (stringChar, afterString) = rest.span { case (strCh, _) =>
            strCh != '"' && strCh != '\n'
          }
          val string = stringChar.map(_._1).mkString
          if (afterString.isEmpty || afterString.head._1 == '\n') {
            ctx.reporter.error("String never closed, special character not escaped", currentPos)
            (BAD().setPos(currentPos), afterString)
          }
          (STRINGLIT(string).setPos(currentPos), afterString.tail) //First character of afterString is '"' so we skip it
        ...
        case '&' =>
          if (nextChar == '&')
            ...
            ctx.reporter.error("Unknown symbol", pos = currentPos)
            useOne(BAD())
          }
        case '|' =>
          if (nextChar == '|')
